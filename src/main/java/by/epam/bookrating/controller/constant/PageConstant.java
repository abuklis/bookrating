package by.epam.bookrating.controller.constant;

/**
 * Class consists of paths to the jsp-pages
 */
public final class PageConstant {
    public static final String ERROR_PAGE = "/jsp/fail.jsp";
    public static final String BOOK_LIST_PAGE = "/jsp/viewAll.jsp";
    public static final String ADD_BOOK_PAGE = "/jsp/addBook.jsp";
    public static final String VIEW_SINGLE = "/jsp/viewSingle.jsp";
    public static final String MAIN_PAGE = "/index.jsp";
    public static final String EDIT_BOOK_PAGE ="/jsp/editBook.jsp";
    public static final String REGISTRATION_PAGE = "/jsp/registration.jsp";
    public static final String LOGIN_PAGE = "/jsp/login.jsp";
    public static final String VIEW_PROFILE_PAGE = "/jsp/profile.jsp";
    public static final String EDIT_PROFILE_PAGE = "/jsp/editProfile.jsp";
    public static final String BOOKS_BY_GENRE = "/jsp/booksByGenre.jsp";
    public static final String VIEW_TODAY_COMMENTS_PAGE = "/jsp/viewTodayComments.jsp";
    public static final String VIEW_USERS_INFO_PAGE = "/jsp/viewAllUsers.jsp";
    public static final String USER_COMMENTS = "/jsp/userComments.jsp";
    public static final String FAVORITE_BOOKS = "/jsp/favoriteBooks.jsp";
    public static final String ADD_AUTHOR_PAGE = "/jsp/addAuthor.jsp";
    public static final String VIEW_AUTHOR = "/jsp/viewAuthor.jsp";
    public static final String EDIT_AUTHOR_PAGE = "/jsp/editAuthor.jsp";
    public static final String READ_BOOKS = "/jsp/readBooks.jsp";
}
